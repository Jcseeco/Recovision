<script>
	...
	function dataPreprocessing(data) {
		...
		seed_customer = {
			...
			inr: INRtoCat.find((c) => seed_data.totalDiscount > c.min && seed_data.totalDiscount <= c.max)
				.category
		};

		//tree data (will use data from aggregation => => change "totalDiscount" to ?)
		var group_data = groups(
			rollups_data,
			(d) => d['Age Group'],
			(d) => d.Gender,
			(d) => findCategory(d.totalDiscount)
		);
		temp_data = {
			name: 'root',
	...
	function buildHierarchy() {
		...
		var partition_ = partition().size([2 * Math.PI, radius * radius]);

		p_root = partition_(root);

		arc_ = arc()
			.startAngle(function (d) {
				return Math.max(0, Math.min(2 * Math.PI, d.x0));
			})
			.endAngle(function (d) {
				return Math.max(0, Math.min(2 * Math.PI, d.x1));
			})
			.innerRadius(function (d) {
				return Math.max(55, Math.sqrt(d.y0));
			})
			.outerRadius(function (d) {
				return Math.max(55, Math.sqrt(d.y1));
			});
	...
	function drawDonuts() {
		...
		donuts
			...
			.style('fill', function (d) {
				...
				if (d.depth == 1) {
					//age group
					temp = SimilaritytoCat(
						Math.abs(AgeGrouptoCat[d.data.name] - AgeGrouptoCat[seed_customer.age])
					);
				} else if (d.depth == 2) {
					//gender
					...
					//inr
					temp = SimilaritytoCat(
						Math.abs(d.data.name.charCodeAt(0) - seed_customer.inr.charCodeAt(0))
					);
				} else {
					temp = 'non-smi';
				}
			...
			.on('mouseenter', function (event, d) {
				...
				donuts
					.selectAll('path')
					.style('filter', (node) =>
						ancestors.includes(node) ? 'brightness(1.2)' : 'brightness(0.5)'
					);
				var percentage_ = ((100 * d.value) / p_root.value).toPrecision(3);
				label.style('visibility', 'visible').select('.percentage').text(`${percentage_}%`);
...
<div class="chart-container">
	<svg id="likeme-donuts" width="100%" viewBox="35 0 300 240"> </svg>
</div>
